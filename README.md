...
> **gallagherrich** (`@suganzi/gallagherrich`) is an up-to-date fork of [telegraf](https://github.com/telegraf/telegraf) — the popular Telegram Bot framework for Node.js.  
> This package tracks the latest Telegram Bot API releases and ships features ahead of the upstream package.  
> It is a drop-in replacement: just swap `npm install telegraf` → `npm install @suganzi/gallagherrich` and change your imports from `'telegraf'` to `'@suganzi/gallagherrich'`.  
...
```ts  
import { Telegraf, RichMessage } from '@suganzi/gallagherrich'  
...
```ts  
import { RichMessage } from '@suganzi/gallagherrich'  
const { RichHTMLBuilder: HTML } = RichMessage  
...
```ts  
new HTML()  
  .pre('npm install @suganzi/gallagherrich', 'bash')  
...
new HTML()  
  ...
  .blockQuote(  
    HTML.bold('Suganzi') + ' supports ' + HTML.marked('highlighted') +  
...
new HTML()  
  .paragraph(  
    'Suganzi ' + HTML.ref('note-1', '[1]') + ' is based on Telegraf ' + HTML.ref('note-2', '[2]') + '.'  
  )  
  .divider()  
  .referenceDefinition('note-1', HTML.url('https://npmjs.com/package/@suganzi/gallagherrich', 'gallagherrich on npm'))  
...
```ts  
import { Markup } from '@suganzi/gallagherrich'  
...
```ts  
import { RichMessage } from '@suganzi/gallagherrich'  
import type { ExtraSendRichMessage, ExtraSendRichMessageDraft } from '@suganzi/gallagherrich/types'  
...
import { Context, Telegraf } from '@suganzi/gallagherrich'  
...
```js  
const { Telegraf } = require('@suganzi/gallagherrich')  
const { message } = require('@suganzi/gallagherrich/filters')  
...
```shellscript  
$ npm install @suganzi/gallagherrich  
```  
  
or  
  
```shellscript  
$ yarn add @suganzi/gallagherrich  
```  
  
or  
  
```shellscript  
$ pnpm add @suganzi/gallagherrich  
```  
...
```js  
import { Telegraf } from '@suganzi/gallagherrich'  
import { message } from '@suganzi/gallagherrich/filters'  
